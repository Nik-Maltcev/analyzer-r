...
- **📂 Данные** — выбор рынка (crypto/stocks/forex), статус БД (тикеры/записи/период), фильтры по датам/тиикерам, предпросмотр. Источник данных — только SQLite БД (запечённая из CSV + ежедневный cron-апдейт).
...
- **Volume vs build-time DB**: Railway монтирует свежий volume поверх `/data`, скрывая build-time `market.db`. Решено: seed-CSV лежит в `/opt/seed/` (вне volume), а `start.sh` при первом запуске (или если DB пуста/повреждена) пересобирает БД из CSV. Логирует каждый шаг в stdout (видно в логах Railway).
